...
public class Arborist {
	...
	public ParseTree growTree(String arg){
		String newArgument="";
		...
		for(int i=0;i<arg.length();i++){
			if(arg.charAt(i)=='(' || arg.charAt(i)==')'){
				...
				newArgument+=arg.charAt(i);
			}
			splitArgument=newArgument.split("\\s");
		}
		
		for(int i=0;i<splitArgument.length;i++){
			tree.grow(splitArgument[i]);
		}
		return tree;
	}
	...
	public static void main(String[] args) {
		...
		ParseTree tree=amy.growTree("(add 5 6)");
